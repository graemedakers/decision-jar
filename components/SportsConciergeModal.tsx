...
import { X, Trophy, MapPin, Loader2, Sparkles, ExternalLink, Plus, Zap, Star, Heart, Lock, User, Users, Info } from "lucide-react";
import { Button } from "./ui/Button";
import { useConciergeActions } from "@/hooks/useConciergeActions";
import { getCurrentLocation } from "@/lib/utils";
import { LocationInput } from "./LocationInput";

interface SportsConciergeModalProps {
    isOpen: boolean;
    onClose: () => void;
    userLocation?: string;
    onIdeaAdded?: () => void;
...
export function SportsConciergeModal({ isOpen, onClose, userLocation, onIdeaAdded, onGoTonight, onFavoriteUpdated }: SportsConciergeModalProps) {
    ...
    // Constants
    const SPORT_OPTIONS = [
        "Tennis", "Golf", "Swimming", "Basketball", "Soccer", "Volleyball",
        "Badminton", "Squash/Racquetball", "Pickleball", "Bowling", "Boxing"
    ];
    ...
    const handleGetRecommendations = async () => {
        setIsLoading(true);
        try {
            const res = await fetch('/api/sports-concierge', {
                method: 'POST',
                    ...
                    >
                        <div className="p-6 border-b border-slate-200 dark:border-white/10 flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">Sports Scout</h2>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">Find courts, clubs, and fields</p>
                                </div>
                            </div>
                            <button
                                onClick={onClose}
                        ...
                        <div className="p-6 overflow-y-auto overflow-x-hidden flex-1 space-y-6 px-7">
                            <div className="space-y-4">
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Preferred Sports</label>
                                        <div className="flex flex-wrap gap-2">
                                            {SPORT_OPTIONS.map((sport) => (
                                                <button
                                                    key={sport}
                                                    onClick={() => toggleSelection(sport, selectedSports, setSelectedSports)}
                                                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${selectedSports.includes(sport)
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Access Type</label>
                                        <select
                                            value={membership}
                                        ...
                                        >
                                            ...
                                            <option value="Public Access">Public / No Membership Needed</option>
                                            <option value="Guest Pass">Guest Pass Available</option>
                                            <option value="Membership Required">Private / Membership Required</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="py-2">
                                ...
                                >
                                    {isLoading ? (
                                        <><Loader2 className="w-5 h-5 animate-spin mr-2" /> Searching Facilities...</>
                                    ) : (
                                        <><Sparkles className="w-5 h-5 mr-2" /> Find Sports Clubs</>
                                    )}
                                </Button>
                            </div>
                            ...
                            {recommendations.length > 0 && (
                                <div ref={resultsRef} className="space-y-4 pt-4">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Top Facilities for You</h3>
                                        <button
                                            ...
                                            <div key={index} className="glass p-4 rounded-xl flex flex-col sm:flex-row gap-4 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors relative bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 shadow-sm dark:shadow-none">
                                                ...
                                                <div className="flex-1 pr-8">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-xs text-slate-400">
                                                        <span className="flex items-center gap-1"><Trophy className="w-3 h-3" /> {rec.sport_type}</span>
                                                        <span className="flex items-center gap-1"><Info className="w-3 h-3" /> {rec.membership_required}</span>
                                                ...
                                                <div className="flex flex-wrap sm:flex-col gap-2 justify-start sm:justify-end">
                                                    ...
                                                    <Button size="sm" onClick={() => handleGoTonight(rec, "ACTIVITY", isPrivate)} className="text-xs bg-gradient-to-r from-emerald-400/20 to-teal-400/20 text-emerald-700 dark:text-emerald-200 border border-emerald-400/30 hover:bg-emerald-400/30">
