...
export function QuickDecisionsModal({ isOpen, onClose }: QuickDecisionsModalProps) {
    ...
    const recordAction = async (action: string, meta?: any) => {
        try {
            const res = await fetch('/api/gamification/record-action', {
                method: 'POST',
            ...
            if (data.success && data.achievements && data.achievements.length > 0) {
                ...
                // data.achievements = [{ unlocked: true, achievement: {...}, xpResult: ... }]
                setUnlockedAchievement(data.achievements[0].achievement);
            }
        } catch (e) {
