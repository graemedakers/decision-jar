...
import { X, MapPin, Loader2, Sparkles, ExternalLink, Plus, Zap, Heart, Leaf, Lock, Star } from "lucide-react";
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {recommendations.map((rec, index) => (
                                            <div key={index} className="glass p-4 rounded-xl flex flex-col sm:flex-row gap-4 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors relative bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 shadow-sm dark:shadow-none">
                                                <button onClick={() => handleFavorite(rec, "WELLNESS")} className={`absolute top-3 right-3 p-2 rounded-full transition-all z-10 ${rec.isFavorite ? 'text-pink-500 bg-pink-500/10' : 'text-slate-400 hover:text-pink-400 hover:bg-slate-100 dark:hover:bg-white/5'}`}><Heart className={`w-5 h-5 ${rec.isFavorite ? 'fill-current' : ''}`} /></button>
                                                <div className="flex-1 pr-8">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-xs text-slate-400">
                                                        <span className="flex items-center gap-1"><Leaf className="w-3 h-3" /> {rec.speciality || 'Wellness'}</span>
                                                ...
                                                <div className="flex flex-wrap sm:flex-col gap-2 justify-start sm:justify-end">
                                                    ...
                                                    <Button size="sm" onClick={() => handleGoTonight(rec, "ACTIVITY", isPrivate)} className="text-xs bg-gradient-to-r from-emerald-400/20 to-teal-400/20 text-emerald-700 dark:text-emerald-200 border border-emerald-400/30">
