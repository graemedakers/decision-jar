                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {recommendations.map((rec, index) => (
                                            <div key={index} className="glass p-4 rounded-xl flex flex-col sm:flex-row gap-4 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors relative bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 shadow-sm dark:shadow-none">
                                                <button onClick={() => handleFavorite(rec, "GAME")} className={`absolute top-3 right-3 p-2 rounded-full transition-all z-10 ${rec.isFavorite ? 'text-pink-500 bg-pink-500/10' : 'text-slate-400 hover:text-pink-400 hover:bg-slate-100 dark:hover:bg-white/5'}`}><Heart className={`w-5 h-5 ${rec.isFavorite ? 'fill-current' : ''}`} /></button>
                                                <div className="flex-1 pr-8">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-xs text-slate-400">
                                                        <span className="flex items-center gap-1"><Gamepad2 className="w-3 h-3" /> {rec.speciality || 'Game'}</span>
                                                        <span className="flex items-center gap-1"><Monitor className="w-3 h-3" /> {rec.address || 'N/A'}</span>
                                                        {rec.google_rating && (
                                                ...
                                                <div className="flex flex-wrap sm:flex-col gap-2 justify-start sm:justify-end">
                                                    ...
                                                    <Button size="sm" onClick={() => handleAddToJar(rec, "GAME", isPrivate)} className="text-xs bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-white hover:bg-slate-200 dark:hover:bg-white/20">
                                                        <Plus className="w-4 h-4 mr-1" /> Jar
                                                    </Button>
                                                    <Button size="sm" onClick={() => handleGoTonight(rec, "GAME", isPrivate)} className="text-xs bg-gradient-to-r from-indigo-400/20 to-purple-400/20 text-indigo-700 dark:text-indigo-200 border border-indigo-400/30">
                                                        <Zap className="w-4 h-4 mr-1" /> Play Now
                                                    </Button>
                                                </div>
                                            </div>
                                        ))}
