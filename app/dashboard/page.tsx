...
import { Wine, Disc, Bed, Clapperboard, Leaf, Dumbbell, Ticket, Gamepad2, Footprints, Key, Trophy, Book } from "lucide-react";
...
function DashboardContent() {
    ...
    const handleLogout = async () => {
        ...
        // Clear custom session via API
        await fetch(getApiUrl('/api/auth/logout'), { method: 'POST' });

        // Final redirect and state refresh
        window.location.href = '/';
    };
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
                                    ...
                                    <motion.div
                                        ...
                                        whileTap={{ scale: 0.98 }}
                                        className="group relative h-full"
                                        onClick={() => isPremium ? setIsBarCrawlOpen(true) : setIsPremiumModalOpen(true)}
                                    >
                                        <div className="absolute -inset-0.5 bg-gradient-to-b from-orange-500/20 to-transparent rounded-3xl opacity-0 group-hover:opacity-100 transition duration-500" />
                                        <div className="relative h-full bg-white dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-white/5 rounded-3xl p-8 flex flex-col gap-6 cursor-pointer hover:shadow-2xl hover:shadow-orange-500/10 transition-all">
                                            <div className="flex items-center justify-between">
                                            ...
                                            <div>
                                                <span className="block text-xl font-bold text-slate-900 dark:text-white mb-2 leading-tight">Bar Crawl Planner</span>
                                                <span className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed block">Design a route for the perfect night out.</span>
                                            </div>
                                        </div>
                                    </motion.div>
                                    ...
                                    <motion.div
                                        ...
                                        whileTap={{ scale: 0.98 }}
                                        className="group relative h-full"
                                        onClick={() => isPremium ? setIsTheatreModalOpen(true) : setIsPremiumModalOpen(true)}
                                    >
                                        <div className="absolute -inset-0.5 bg-gradient-to-b from-purple-500/20 to-transparent rounded-3xl opacity-0 group-hover:opacity-100 transition duration-500" />
                                        <div className="relative h-full bg-white dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-white/5 rounded-3xl p-8 flex flex-col gap-6 cursor-pointer hover:shadow-2xl hover:shadow-purple-500/10 transition-all">
                                            <div className="flex items-center justify-between">
                                    ...
                                    <motion.div
                                        ...
                                        whileTap={{ scale: 0.98 }}
                                        className="group relative h-full"
                                        onClick={() => isPremium ? setIsBookModalOpen(true) : setIsPremiumModalOpen(true)}
                                    >
                                        <div className="absolute -inset-0.5 bg-gradient-to-b from-blue-500/20 to-transparent rounded-3xl opacity-0 group-hover:opacity-100 transition duration-500" />
                                        <div className="relative h-full bg-white dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-white/5 rounded-3xl p-8 flex flex-col gap-6 cursor-pointer hover:shadow-2xl hover:shadow-blue-500/10 transition-all">
                                            <div className="flex items-center justify-between">
                                                <div className="w-14 h-14 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-600 dark:text-blue-400 ring-1 ring-blue-500/20 group-hover:bg-blue-500 group-hover:text-white transition-all duration-500 shadow-inner">
                                                    <Book className="w-7 h-7" />
                                                </div>
                                                {!isPremium && <Lock className="w-5 h-5 text-slate-300 dark:text-slate-600" />}
                                            </div>
                                            <div>
                                                <span className="block text-xl font-bold text-slate-900 dark:text-white mb-2 leading-tight">Book Finder</span>
                                                <span className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed block">Discover your next favorite read tailored to your mood.</span>
                                            </div>
                                        </div>
                                    </motion.div>
                                </div>
